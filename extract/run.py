...
def ims_try_for_top_candidate(
    ...
    # 2) Try other ISO2 hints (excluding own_iso if present)
    for iso2 in search_iso2s:
        iso2_u = (iso2 or "").upper()
        if not iso2_u or iso2_u == (own_iso or "").upper():
            continue
        fac, st, why = _one(iso2_u)
        if fac:
            return fac, st, why
...
_BAD_PORT_SPANS = {
    "port", "ports", "harbour", "harbor", "since", "tugs", "salvage",
}
...
def run():
    ...
    print("[RUN] extract.run v3.4 — Option B: country+region ports, no guessing without spans")
    ...
    place_resolver = PlaceResolver(
        str(UNLOCODE_CSV), str(GEONAMES_CSV), str(COUNTRY_CSV), str(REGIONS_JSON)
    )
    ...
    for cf in incident_files:
        ...
        if DEBUG_NER or DEBUG_PIPE:
            ...
            print(f"ISO2 from names: {name_hints}")
            print("==========================================\n")
        ...
        _dbg_pipe("[PIPE] calling PlaceResolver.build_candidates_for_article(...)")

        # ---- Port candidates (from PlaceResolver, Option B)
        pr_hits_raw = place_resolver.build_candidates_for_article(
            ports_text=ports_text,
            country_iso2s=search_iso2s,
            region_names=regions_text,
        )

        _dbg_pipe(f"PlaceResolver returned {len(pr_hits_raw)} candidates")
        ...
        # region centroid (for distance feature)
        def infer_region_centroid() -> Optional[Tuple[float, float]]:
            if not regions_text:
                return None
            for r in regions_text:
                e = region_centroids.get(r) or {}
        ...
        region_center = infer_region_centroid()
        ...
        for h in pr_hits:
            span = h.get("locationName", "") or h.get("unlocode", "") or ""
            spans.append(span)
